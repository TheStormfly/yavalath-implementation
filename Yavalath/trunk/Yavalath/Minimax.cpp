...
double Minimax::Negamax(structBoard game, double alpha, double beta, int depth, short int color)
{
		...
		if(depth == 0)
		{
			double j=Evaluate(b.b,color);
			return (j);
		}
		else
		{
			if(empDoBoard->IsGameOver(b.b)==OurC)//ve se é vitoria
			{
				return 999999;	
			}
			if(empDoBoard->IsGameOver(b.b)==OurE)//ve se é derrota
			{
				return -999999;	
			}
			else
			{
				...
				this->JogadasPossiveis(b, &child,OurC);
...
void Minimax::Play(structBoard board1)
{
	...
	//ve uma jogada possivel e diz que é o melhor best move
	int p=0;
	bool exit= false;
	while(exit==false)
	{
		if(b.b[p]='0')//se esta vazio
		{
			this->bestMove=p;
			exit=true;
		}
	...
	for(short int i=0; i <(short int)filhos.size(); i++) // para cada filho, chama o negamax
	{
		b.b[filhos[i]]= OurC;
		g= -Negamax(b, -9999, 9999, 1, enemyColor);
		b.b[filhos[i]]= '0';
		if(g > bestValue)
		{
			bestValue = g;
